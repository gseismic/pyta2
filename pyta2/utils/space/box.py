...
class Box(Space):
    ...
    def __init__(
        self, low, high, shape=None, dtype=np.float64, null_value=None, seed=None
    ):
    ...
    def sample(self):
        if np.issubdtype(self.dtype, np.integer):
            ...
            return self.np_random.uniform(low=self.low, high=self.high, size=self.shape)
    ...
    def is_null(self, x):
        ...
        return (
            np.isnan(x)
            if (np.isnan(self.null_value) and np.issubdtype(self.dtype, np.floating))
            else np.equal(x, self.null_value)
        )
    ...
    def __eq__(self, other):
        if not isinstance(other, Box):
            return False
        null_equal = (
            (self.null_value is None and other.null_value is None)
            or (
                self.null_value is not None
                and other.null_value is not None
                and np.isnan(self.null_value)
                and np.isnan(other.null_value)
            )
    ...
    def __repr__(self):
        return (
            f"Box(low={self.low}, high={self.high}, "
...
class PositiveScalar(PositiveBox):
    ...
    def __init__(self, high=None, dtype=np.float64):
        super(PositiveScalar, self).__init__(high=high, shape=(), dtype=dtype)
...
class NegativeScalar(NegativeBox):
    ...
    def __init__(self, low=None, dtype=np.float64):
        super(NegativeScalar, self).__init__(low=low, shape=(), dtype=dtype)
...
class Scalar(Box):
    ...
    def __init__(self, high=None, low=None, dtype=np.float64, **kwargs):
        super(Scalar, self).__init__(
            high=high, low=low, shape=(), dtype=dtype, **kwargs
        )
