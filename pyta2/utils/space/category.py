...
class Category(Space):
    ...
    def sample(self, mask=None):
        """
        从类别中随机采样
        :param mask: 可选掩码，形状为(n,)的np.int8数组，1表示允许选择，0表示排除
        """
        if mask is not None:
            ...
            idx = self.np_random.integers(0, self.n)
    ...
    def __repr__(self):
        return f"Category(cats={self.categories}, dtype={self.dtype})"
    ...
    def from_json(cls, data):
        """从JSON反序列化"""
        return cls(cats=data["categories"])
